...
export const getSuperclassForwardConnections = async (
  ...
  for (const relation of relations) {
    ...
    connection.subtrees = await getSuperclassForwardConnections(
      conn,
      subparentId,
      EntityEnums.Class.Concept,
      nestLvl + 1
...
> => {
  ...
  if (asClass === EntityEnums.Class.Location) {
    ...
    for (const relation of relations) {
      ...
      connection.subtrees = await getSuperordinateLocationForwardConnections(
        conn,
        subparentId,
        asClass,
        nestLvl + 1
...
export const getSubjectActant1ReciprocalForwardConnections = async (
  ...
  RelationTypes.IConnection<RelationTypes.ISubjectActant1Reciprocal>[]
> => {
...
export const getIdentificationForwardConnections = async (
  ...
  for (const relation of relations) {
    ...
    if (!thresholdReached) {
      ...
      connection.subtrees = await getIdentificationForwardConnections(
        conn,
        subparentId,
        nextThreshold,
        nestLvl + 1
