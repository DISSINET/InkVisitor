# A-C-R export "acr-09_2026"

Generated 2026-09-08T17:32:03.185Z from `inkvisitor`.

- entities: **22,775** (22,775 distinct ids)
- relations: **12,439**
- audits: **50,961**
- entities carrying at least one audit: 13,984; without any: 8,791

## Entity classes

| class | name | count | share |
| --- | --: | --: | --: |
| `V` | Value | 13,984 | 61.40% |
| `C` | Concept | 7,594 | 33.34% |
| `A` | Action | 1,188 | 5.22% |
| `R` | Resource | 9 | 0.04% |

## Relation types

| type | count | share |
| --- | --: | --: |
| `SCL` | 6,979 | 56.11% |
| `SUS` | 1,137 | 9.14% |
| `AEE` | 1,028 | 8.26% |
| `A1S` | 993 | 7.98% |
| `REL` | 783 | 6.29% |
| `SYN` | 588 | 4.73% |
| `A2S` | 446 | 3.59% |
| `PRR` | 172 | 1.38% |
| `SAR` | 156 | 1.25% |
| `HOL` | 60 | 0.48% |
| `ANT` | 48 | 0.39% |
| `IMP` | 33 | 0.27% |
| `CLA` | 12 | 0.10% |
| `SOE` | 4 | 0.03% |

## Authorship

Shares are of all creations, of all edits, and of all audit records in this dataset. Roles are not assigned here.

| user | id | creations | of creations | edits | of edits | all | of all |
| --- | --: | --: | --: | --: | --: | --: | --: |
| David Zbíral | `100` | 5,224 | 37.36% | 10,828 | 29.28% | 16,052 | 31.50% |
| Soňa Hudíková | `6dc46dfb-af6f-4726-a3d9-65185ae958f9` | 1,301 | 9.30% | 8,636 | 23.36% | 9,937 | 19.50% |
| Katia Riccardo | `103` | 2,528 | 18.08% | 4,772 | 12.91% | 7,300 | 14.32% |
| Robert Shaw | `101` | 1,170 | 8.37% | 4,220 | 11.41% | 5,390 | 10.58% |
| Katalin Suba | `02868c93-517e-4602-94b7-057ce1a393fd` | 883 | 6.31% | 4,245 | 11.48% | 5,128 | 10.06% |
| stanislaw.banach | `5a8b6aff-3dc1-40b2-9704-82b47a952d4c` | 972 | 6.95% | 3,593 | 9.72% | 4,565 | 8.96% |
| Davor Salihovic | `102` | 1,073 | 7.67% | 164 | 0.44% | 1,237 | 2.43% |
| Sari Katajala | `bfb284c6-7180-4ae7-a5ef-e27f91121fd3` | 486 | 3.48% | 168 | 0.45% | 654 | 1.28% |
| admin | `1` | 35 | 0.25% | 182 | 0.49% | 217 | 0.43% |
| Reima Välimäki | `105` | 65 | 0.46% | 138 | 0.37% | 203 | 0.40% |
| Larissa de Freitas Lyth | `107` | 198 | 1.42% | 1 | 0.00% | 199 | 0.39% |
| Jenni Kuuliala | `9ec0df8f-0f4e-459b-b847-7ad916caa417` | 37 | 0.26% | 16 | 0.04% | 53 | 0.10% |
| Saku Pihko | `18fd8891-5824-47a7-bad4-4544bc23f370` | 12 | 0.09% | 7 | 0.02% | 19 | 0.04% |
| Tomáš Hampejs | `151` | 0 | 0.00% | 7 | 0.02% | 7 | 0.01% |

## Warning: metaprops not carried into this dataset

74 metaprops on exported entities were dropped, because a prop can only be carried when both of its endpoints are in the dataset: 20 unfinished, 54 outOfScope. Each row names the entity carrying the prop, so it can be opened in the app by id or label.

### Endpoint left empty

The prop carries no id on one side, so it was never finished in the editor. Nothing is lost by dropping it.

| carrier | carrier id | prop type | prop value |
| --- | --: | --: | --: |
| C occupation in finance | `002fdd38-a74d-4a92-9216-1a1baba08cbe` | —  (unfinished) | finance `7a0789a8-5769-4969-83f1-d19db6215d57` (present) |
| C communicative intention | `5470c201-eba4-42f9-9b62-3663599db987` | —  (unfinished) | —  (unfinished) |
| C meeting heretics | `58fd5f20-0957-44c7-a167-0685456f4464` | —  (unfinished) | hereticus/a `3438609f-b809-4be1-888c-a53df7bb7c1c` (present) |
| C ter flexis genibus | `5fb52fa6-a585-41d4-be0c-84c40b6bf07a` | —  (unfinished) | —  (unfinished) |
| C God as a father metaphor | `89c62d4b-9f95-4f9a-bc39-123743b205c3` | father functor `9dc91410-2c66-4a7b-b4d4-4b9188740638` (present) | —  (unfinished) |
| C God as a father metaphor | `89c62d4b-9f95-4f9a-bc39-123743b205c3` | child functor `da32d596-8821-4982-8cd9-e43f3ecbe6d1` (present) | —  (unfinished) |
| C God as a father metaphor | `89c62d4b-9f95-4f9a-bc39-123743b205c3` | —  (unfinished) | —  (unfinished) |
| C perverse opiniones quas Ricardus Gest et Johannes predicant et favent | `a1ca3e57-ef5d-41f2-9e20-3b78de8046c6` | —  (unfinished) | —  (unfinished) |
| C meeting participant | `a7d0e79c-b7cf-4d28-9965-aab9186729e3` | —  (unfinished) | —  (unfinished) |
| C transcribed text | `a0622896-892c-4ceb-9961-987e6a211eaf` | —  (unfinished) | —  (unfinished) |
| C iuratus/a | `9bdba789-ea64-4b74-bc1d-adedf43653eb` | —  (unfinished) | —  (unfinished) |
| C sitting together as social interaction | `aad86400-a309-4888-860e-8b985c5784f6` | derived meaning `2b5eef80-4962-4e3c-96c5-d79051686a43` (present) | —  (unfinished) |
| C originator | `df0c2ea2-c109-4991-a859-8806ac2304be` | —  (unfinished) | —  (unfinished) |
| C sitting | `d2d95c22-b991-4d8b-aae7-5aa39d3b7949` | derived meaning `2b5eef80-4962-4e3c-96c5-d79051686a43` (present) | —  (unfinished) |
| C seditiosus/a/um | `d562a462-c309-4ae0-9adc-0200a3c37a29` | —  (unfinished) | —  (unfinished) |
| C manus in terra [positae] | `f91c53c2-a14f-4446-ba08-7e842f21288e` | —  (unfinished) | —  (unfinished) |
| A quesivit | `040768d0-cc96-4e1b-aabe-31e2cc706c2e` | —  (unfinished) | —  (unfinished) |
| A testatus/a | `0e8166b4-f424-4717-b29e-685899e1b3c3` | —  (unfinished) | —  (unfinished) |
| A dedit penitentiam | `273362ff-b3ba-4d54-a80a-06abc77badb7` | —  (unfinished) | penitentia `b35dfc9b-57e0-47d8-80f3-dba9eb03fc84` (present) |
| A accepit penitentiam | `88c0def4-d5bf-416e-9cbf-3c97cf87d79c` | —  (unfinished) | penitentia `b35dfc9b-57e0-47d8-80f3-dba9eb03fc84` (present) |

### Endpoint of a class this dataset does not cover

The scope working as designed: the prop points at an entity of a class outside the dataset, so neither can be carried.

| carrier | carrier id | prop type | prop value |
| --- | --: | --: | --: |
| C consuetudo credentium hereticorum | `00b67cd5-a710-4696-ad57-b678db1f7dc4` | participant `9d4e97b0-9cbd-4dbe-9760-f132616d97d5` (present) | credentes hereticorum `f87d51ef-38b1-41e5-b313-586f85ce3170` (outOfScope) |
| C abbas Crassensis | `00b60c17-2a3e-4e19-bb7d-bc0333c9ef25` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Monasterium Crassense `5b3a33d7-cfe2-4af5-b70c-df59d2e4368f` (outOfScope) |
| C minor archidiaconus Carcassonae | `0de054fd-3e09-48f2-86b0-19b4c573ebc4` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Carcassona `8fdfecde-ded9-4939-a30f-5a74d406d857` (outOfScope) |
| C auxilium beate Katharine | `131e257c-1118-4e85-a3bf-109b592468e5` | grantor `80a00844-00a8-4db3-984a-5d6af8bda762` (present) | Katharina of Vadstena `29e0779e-3acc-4fcb-8889-33523971afe4` (outOfScope) |
| C prior de manso Sanctarum Puellarum | `10460611-45ea-43da-8193-4aac783628ca` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Mansum Sanctarum Puellarum `47aa282d-55b0-4dd6-baaf-54b0c6fc972c` (outOfScope) |
| C episcopus Coventrie et Lichfeldie | `1cbe89df-932e-4151-890f-2b3f78529fa6` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Coventria `49af5a6a-b2cb-438b-88c9-f9ae02b79b29` (outOfScope) |
| C episcopus Coventrie et Lichfeldie | `1cbe89df-932e-4151-890f-2b3f78529fa6` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Lichfeldia `66c10dd5-1001-4ab9-a81d-8d282231c6a9` (outOfScope) |
| C bishop of Norwich | `1b4744a2-6e54-4966-a6b2-2abeba595fe9` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | diocese of Norwich `ec906856-6f91-49d6-a9e6-99afc545c317` (outOfScope) |
| C comes Montisfortis | `21733cd9-3b9b-44ce-a95c-83feabbcfe34` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Monsfortis `86262b22-88c3-4112-bf46-2e37c52edf6c` (outOfScope) |
| C bishop of Coventry and Lichfield | `3b9fb0e5-774a-49b1-b61e-0d29c4b6386f` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | diocese of Coventry and Lichfield `f8ab648f-d446-49f2-820c-f808d127c6a1` (outOfScope) |
| C opiniones heretice | `3d70ad7e-6f8b-488b-91c9-1ab57f792752` | amount of singular parts `ea8495b0-31eb-47d0-9de0-9a0389be7a5e` (present) | >1 `97e40cda-f35b-437b-b0e6-c1684070fbbf` (outOfScope) |
| C prior sanctae Mariae suburbii Carcassonae | `46a18300-09f1-4fdf-98a4-e4f612c34339` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | sancta Maria suburbii Carcasonae `40c84288-9c99-4dea-a3e7-511136b3f0c7` (outOfScope) |
| C prepositus Tholosane | `4342e1be-4563-441d-8781-57e8c90ed91f` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Tholosae [diocesis] `9ee41dfa-e9a6-46c6-9e01-de29681a79f2` (outOfScope) |
| C archidiaconus Redensis | `4cc0412b-730e-4f02-b8a3-94fd3fc6b563` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | [pagus] Redensis `7e43ffd5-ee81-4ece-b089-0ecd11436e86` (outOfScope) |
| C comes Tholosanus | `4eb53784-767e-4f26-ad18-a35acd4a922d` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Tholose [comitatus] `177f0977-8c2d-44dd-aa87-4f3c3d86096c` (outOfScope) |
| C figura sancte Guillelme | `53e3aeec-edf6-4fe6-804f-0fab09bf82e0` | topic `eec5ad41-0fb2-4669-a48c-89930130963a` (present) | sancta Guillelma `62f08cee-80a9-4c0d-9d11-97f0b99cbad6` (outOfScope) |
| C peccata falsorum christianorum et illorum qui crucifixerunt Christum | `6221f251-85d4-43eb-a9af-ecd34d35c586` | perpetrator `b8bdafe1-7f78-4f1b-89ab-88b722de6153` (present) | falsi christiani `5aa35723-efa6-48c7-82e1-0177bb0a7bc7` (outOfScope) |
| C peccata falsorum christianorum et illorum qui crucifixerunt Christum | `6221f251-85d4-43eb-a9af-ecd34d35c586` | perpetrator `b8bdafe1-7f78-4f1b-89ab-88b722de6153` (present) | illi qui crucifixerunt Christum `dc363a44-6fa3-4178-9f6a-687e588036a1` (outOfScope) |
| C episcopus Carcassonae | `63d6bf00-80f6-488f-b78d-8769f378acf9` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Carcassona `8fdfecde-ded9-4939-a30f-5a74d406d857` (outOfScope) |
| C bishop of Passau | `6df88583-4330-4cee-b552-dfb9f44b64f3` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Passau `7e19cbcf-f891-4a3d-8897-a2c06366b0e4` (outOfScope) |
| C archidiaconus Veterismoresii | `80c57279-59cb-4a40-bb15-68ad71d8de4c` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Vielmorès `dd9b7cee-ef1d-4c06-81a5-051f9c20acf3` (outOfScope) |
| C opiniones [Roberti Hachet] | `80507bb5-4949-4351-8de2-af1ddd513798` | amount of singular parts `ea8495b0-31eb-47d0-9de0-9a0389be7a5e` (present) | >1 `19cc8a36-3b60-4aa1-96f4-010741611aa9` (outOfScope) |
| C archiepiscopus Narbonensis | `82614b48-6057-4d4e-ad72-6f2f8b92c8db` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Narbonensis [diocesis] `54018ad0-2e46-49a3-82a3-6ee561f82f3e` (outOfScope) |
| C maior gratia et virtus et auctoritas quam habuerit Petrus apostolus | `8b9efb95-17ac-4c99-981d-edd4410ea44b` | CRP more `1fb9c31d-a58f-4720-b298-7ce2e56c6b39` (present) | Petrus apostolus `ab47724a-4f48-4507-8eae-51cd1d33dd16` (outOfScope) |
| C abbas Soricinensii | `95435b44-a93d-4c2b-89a6-147c04a0ac86` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Monasterium Sanctae Mariae Soricinensis `5f9f221c-5752-46d1-859b-afcb48e29927` (outOfScope) |
| C bishop of Quimper and Léon | `90429959-6eb3-47d8-986b-569570e3fbe4` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | diocese of Quimper and Léon `635ad443-5980-4bbe-9415-38ccd5dbac58` (outOfScope) |
| C bishop of Lincoln | `ab43ed61-2a9c-4704-a5ed-9ca1731aa2a5` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | diocese of Lincoln `35c2da01-11af-4844-8032-84f7dab93015` (outOfScope) |
| C senescallus Carcassonae | `9d9900d6-1304-4be3-bac0-5abf36747c78` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Carcassona `8fdfecde-ded9-4939-a30f-5a74d406d857` (outOfScope) |
| C rex Francie | `b365640e-478c-4a38-a364-b028e0bef246` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Francia `bdc31b13-82b3-4e87-82aa-77003ecc95df` (outOfScope) |
| C repeatedly | `9a4f88fe-3b1a-4826-b189-f927fe9bd81b` | frequency `78acff75-5d74-42c3-a852-d7de6010b98a` (present) | >1 `4cde171c-a9e6-4331-9831-b99301d98b41` (outOfScope) |
| C baiulus de Dozincho | `a86b1044-9929-4087-95dc-b3adef179b41` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Dozinchum `d8474d55-576a-433b-ae31-52f9af4f490c` (outOfScope) |
| C abbas Sancti Papuli | `a3cfe17e-afad-4463-80e7-93166bad8559` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Monasterium Sancti Papuli `3f1ccb6c-20b7-46e1-8fc2-1e28ca2e2e94` (outOfScope) |
| C abbas Montisolivi | `9c7c8595-af99-4f0b-b0dd-e6c528d1a2ee` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Montisolivi [abbatia] `df26f21f-af3e-4189-a914-d34e75235553` (outOfScope) |
| C canonicus Appamiarum | `b92ec623-f89e-4a05-9041-1869fd63313d` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Ecclesia Appamiae `a9eff20a-8912-4eaa-a6db-8533cab224a3` (outOfScope) |
| C canonicus Appamiae | `b86a5f18-0fef-4af5-96a7-75d7014a1138` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Ecclesia Appamiae `a9eff20a-8912-4eaa-a6db-8533cab224a3` (outOfScope) |
| C abbas Villaelongae | `be547f6d-534a-4ad9-a8ff-061654a93b3b` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Villalonga `d87e01ae-9bde-44ad-ba1d-12b7f7403929` (outOfScope) |
| C archidiaconus Carcassone | `b268148d-3ad0-4303-ba1f-e7fce0b08149` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Carcassone [diocesis] `a51bcefa-f8c0-48d5-a175-33340fb754cc` (outOfScope) |
| C una nox | `da2d9e1e-d571-4c07-a746-492787becd55` | amount of singular parts `ea8495b0-31eb-47d0-9de0-9a0389be7a5e` (present) | 1 `b9873341-6180-4b07-a7df-b0b1467e5151` (outOfScope) |
| C una nox | `da2d9e1e-d571-4c07-a746-492787becd55` | time span in days `35c9ebf8-29eb-40be-ad79-8224f9df1860` (present) | <1 `fd3a0284-0525-499a-9e7d-9bbb5fbee4e4` (outOfScope) |
| C amor Spiritus sancti | `b8631860-7017-4702-afc2-862c49a8046b` | human object of love `ed64a7d1-4e49-42b5-b27a-2097cf4bc857` (present) | Spiritus sanctus `0e342b41-c5b1-4a00-a88b-9c31032c9687` (outOfScope) |
| C abbas Appamiae | `d6c6268c-63d2-4259-8c2f-3bd71e3f5475` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Monasterium Sancti Antonini `94bbc99d-8d05-44c7-b931-762334114ff8` (outOfScope) |
| C episcopus Tholosane | `d9663e7c-8451-4c3f-89ac-bf448bb0b451` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Tholosae [diocesis] `9ee41dfa-e9a6-46c6-9e01-de29681a79f2` (outOfScope) |
| C sacrista Narbonae Ecclesiae | `d8b372dc-af68-4569-b60a-ba947e4e5cd1` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Ecclesia Narbonae `013b07cb-8c51-4fe9-96a7-a6e0d0afba69` (outOfScope) |
| C episcopus Tudensis | `e16bf26f-da31-454c-8be6-07f36730e19a` | area of authority `73238f30-0d57-4642-8586-960c1aff19f9` (present) | diocesis Tudensis `e5ce8438-a6d0-4848-991a-ab3094ad89f8` (outOfScope) |
| C bishop of Pamiers | `dc7bdb9e-b66d-4902-858b-112600e98bdb` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Pamiers `0a9d93bf-5899-4982-9344-a6266ade76d1` (outOfScope) |
| C doctrine of the Apostles | `ef6d2a8a-572d-49f9-a6c9-9f4d534c12e5` | attitude holder `b2a4b3a1-4f69-4eae-93d9-3281823aa618` (present) | (pseudo-)Apostles `7503e256-ef62-48d6-bae6-8a9dec1af510` (outOfScope) |
| C doctrina Zacharie de Sancta Agata | `e5fccc11-7e9f-4127-a630-44449d855892` | attitude holder `b2a4b3a1-4f69-4eae-93d9-3281823aa618` (present) | Zacharias Bondi Balbi de S. Agata `86c3f4fa-93a5-4b23-af64-e81c5cee282e` (outOfScope) |
| C receptator/receptatrix [apostolorum] | `d81e5179-5f8d-4cb2-a27c-1bb6b9fc0a90` | guest `cb93dc45-62a8-4c26-a664-f00c6c39efcd` (present) | apostoli `348a8f7c-b7e8-4d6b-89d9-0872339fe75e` (outOfScope) |
| C canonicus Tholosanus | `ed6ec8fc-0e73-4a5f-90f6-8fedb44c9246` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Tolosa `add373d7-def8-46c4-a0af-2a7f0613c43b` (outOfScope) |
| C archidiaconus Corbeira | `f0440e2d-dbb7-4743-a6cc-fac754ddac9d` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | [pagus] Corbeira `cf433d03-7d41-46fc-8dd2-87b670e4e7cc` (outOfScope) |
| C maior archidiaconus Narbonae | `db5cd56f-fb15-4efc-865f-aa23671b9c6d` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | Narbona `156bf696-4218-4ce5-b24b-033b56dd919f` (outOfScope) |
| C devotio sancte Guillelme | `f8f41ad2-fd26-4ef6-9920-dbd8db96eeaf` | object of devotion `7174c857-82c9-4dec-8b23-08709e4d17b7` (present) | sancta Guillelma `62f08cee-80a9-4c0d-9d11-97f0b99cbad6` (outOfScope) |
| C anti-Christian polemic | `f88a5c05-08dd-439e-87e1-614f58a51237` | target of polemical attack `369fb3e7-aa01-4f07-aee9-ea2d452c76c5` (present) | Christians `080ce841-1dd6-4732-bcf9-1fad72da2461` (outOfScope) |
| C prior Pruliani | `fab0acce-9134-4455-ac6e-724107025090` | spatial area of authority `0e6c3830-e4d3-4a30-a54d-759f5a59474e` (present) | prioratus Pruliani `680ee1ff-d1fb-4484-8616-d55742d2cc2e` (outOfScope) |
