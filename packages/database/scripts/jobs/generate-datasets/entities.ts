...
export default class EntitiesGenerator {
  ...
  generate() {
    ...
    const actionsSize = getRandomNumber(
      0,
      Math.min(maxAllocatableSize, remainingSize * 0.5)
    );
    this.generateActions(actionsSize);
    remainingSize -= actionsSize;

    const statementsSize = getRandomNumber(
      0,
      Math.min(maxAllocatableSize, remainingSize * 0.5)
    );
    ...
    const resourcesSize = getRandomNumber(
      0,
      Math.min(maxAllocatableSize, remainingSize * 0.5)
    );
    ...
    const personsSize = getRandomNumber(
      0,
      Math.min(maxAllocatableSize, remainingSize * 0.5)
    );
    ...
    const beingsSize = getRandomNumber(
      0,
      Math.min(maxAllocatableSize, remainingSize * 0.5)
    );
    ...
    const groupsSize = getRandomNumber(
      0,
      Math.min(maxAllocatableSize, remainingSize * 0.5)
    );
    ...
    const objectsSize = getRandomNumber(
      0,
      Math.min(maxAllocatableSize, remainingSize * 0.5)
    );
    ...
    const conceptsSize = getRandomNumber(
      0,
      Math.min(maxAllocatableSize, remainingSize * 0.5)
    );
    ...
    const locationsSize = getRandomNumber(
      0,
      Math.min(maxAllocatableSize, remainingSize * 0.5)
    );
    ...
    const valuesSize = getRandomNumber(
      0,
      Math.min(maxAllocatableSize, remainingSize * 0.5)
    );
  ...
  generateStatements(statementsSize: number) {
    ...
    for (let i = 0; i < statementsSize; i++) {
      const territory =
        territories[Math.ceil(Math.random() * territories.length) - 1];
      statements.push(
        new Statement({
          id: this.getNextId(),
          class: EntityEnums.Class.Statement,
          labels: [`Statement ${i + 1}`],
          data: {
            territory: {
              order: -1,
