...
// forked from design-sync lib/source-kit.mjs — group every component from .design-sync/groups.json
...
// Group per component, keyed by name, from .design-sync/groups.json. The
// dir-derived group further down mirrors the source tree, which spreads one
// visual family over several directories — Modal's parts, the two icon button
// group folders — and a doc-frontmatter `category` only wins over a group that
// came out generic. The picker sections in claude.ai/design are grouped by
// family instead, so the map is the last word.
const GROUP_MAP = existsSync(resolve('.design-sync', 'groups.json'))
  ? JSON.parse(readFileSync(resolve('.design-sync', 'groups.json'), 'utf8'))
  : {};
...
export async function resolvePackage(ctx) {
  ...
  for (const c of components) {
    if (GROUP_MAP[c.name]) c.group = slug(GROUP_MAP[c.name]);
  }
