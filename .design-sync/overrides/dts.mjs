// forked from design-sync lib/dts.mjs — props interfaces are named after the component, not <Name>Props
...
export function propsBodyFor(name, ctx) {
  ...
  // This repo names a component's props interface after the component itself
  // rather than `<Name>Props`, so the lookup above misses for most of them.
  // The interface and the component share a name but live in different
  // declaration spaces, so the pair is what identifies it: an interface or
  // alias `<Name>` in the same file that declares the value `<Name>`.
  if (!decl) {
    for (const sf of project.getSourceFiles()) {
      const fp = sf.getFilePath();
      if (!fp.startsWith(pkgDir)) continue;
      if (/\/(deprecated|legacy|experimental)\//i.test(fp)) continue;
      const cand = sf.getInterface(name) ?? sf.getTypeAlias(name);
      if (cand && sf.getVariableDeclaration(name)) { decl = cand; break; }
    }
  }
